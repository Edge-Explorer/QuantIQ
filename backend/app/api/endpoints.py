...
async def get_model_metrics(db: AsyncSession = Depends(get_db)):
    ...
    # 1. Prediction log summary counts
    try:
    ...
    except Exception as db_err1:
        print(f"Metrics warning (prediction counts): {db_err1}")

    # 2. Prediction log performance by version
    try:
        version_stmt = (
            select(
        ...
        print(f"Metrics warning (version breakdown): {db_err2}")

    # 3. Recent prediction logs
    try:
        ...
        print(f"Metrics warning (recent logs): {db_err3}")

    # 4. Strategy Log Statistics (User vs AI comparison)
    try:
    ...
    except Exception as db_err4:
        print(f"Metrics warning (strategy logs): {db_err4}")
